	...
	function imprimirResumen(res){
		const html = [
			'<div style="font-family:Arial,sans-serif;font-size:12px">',
				...
				// Linea de empresa
				('<div><strong>'+esc(localStorage.getItem('empresaNombre')||'')+'</strong>'+(localStorage.getItem('empresaNit')? ' - CUIL/CUIT: '+esc(localStorage.getItem('empresaNit')):'')+'</div>'),
				...
				(res.stats?('<div>Autos: '+(res.stats.porTipo&&res.stats.porTipo.auto||0)+' | Camionetas: '+(res.stats.porTipo&&res.stats.porTipo.camioneta||0)+' | Motos: '+(res.stats.porTipo&&res.stats.porTipo.moto||0)+'</div>'):''),
				'<hr/>',
